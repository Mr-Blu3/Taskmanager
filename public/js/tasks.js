		...
		$.each(response, function (project, items) {
			...
			$.each(items, function (order, item) {
				if (typeof item != 'undefined') {
					projectString += '<div id="' + item._id + '" class="item prio-' + item.prio + '" style="width: 100%;height:' + getHeight(item.days) + 'px"><img class="left_pen" src="/images/edit.png" /><span class="item_title">' + item.title + '</span><img class="right_pen" src="/images/edit.png" /></div>';
					if (item.deadline)
						projectString += '<div class="deadline"><img src="/images/deadline.png" />' + item.deadline + '</div>';
				}
